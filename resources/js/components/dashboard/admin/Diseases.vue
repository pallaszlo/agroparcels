<template>
  <div>
    <section-header text="Betegségek" />
    <v-data-table
      :headers="headers"
      :items="diseases"
      :search="search"
      ...
      <template v-slot:top class="toolbar">
       <div class="toolbar">
          ...
          <v-btn
            ...
            dark
            @click="showNewItem()">
            <v-icon dark left>mdi-plus</v-icon>Új Betegség
          </v-btn>
       </div>
    ...
    <v-dialog v-model="show_dialog" width="750px">
     <v-card>
        <v-toolbar dense dark color="#292929">
          <v-toolbar-title>Betegség adatai</v-toolbar-title>
        </v-toolbar>
    ...
    <v-dialog v-model="show_dialog2" width="750px">
      <v-card>
        <v-toolbar dense dark color="#292929">
          <v-toolbar-title>Új Betegség</v-toolbar-title>
        </v-toolbar>
...
export default {
  ...
  methods: {
    ...
    updateItem: function () {
      axios
        .put("api/diseases/" + this.showedItem.uuid, {
    ...
    editItem(item) {
      this.showedIndex = this.diseases.indexOf(item);
