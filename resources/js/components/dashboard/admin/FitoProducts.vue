<template>
  <div>
    <section-header text="Növényvédők" />
    <v-data-table
      :headers="headers"
      :items="fitoproducts"
      :search="search"
      ...
      <template v-slot:top class="toolbar">
       <div class="toolbar">
          ...
          <v-btn
            ...
            dark
            @click="showNewItem()">
            <v-icon dark left>mdi-plus</v-icon>Új Növényvédő
          </v-btn>
       </div>
    ...
    <v-dialog v-model="show_dialog" width="750px">
      <v-card>
        <v-toolbar dense dark color="#292929">
          <v-toolbar-title>Növényvédő adatai</v-toolbar-title>
        </v-toolbar>
        ...
        <v-card-actions>
          ...
          <v-btn color="red darken-1" outlined @click.native="close"
            >Bezár</v-btn
          >
          <v-btn color="red darken-1" outlined @click.native="validateItem"
            >Növényvédő frissítése</v-btn
          >
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="show_dialog2" width="750px">
      <v-card>
        <v-toolbar dense dark color="#292929">
          <v-toolbar-title>Új Növényvédő</v-toolbar-title>
        </v-toolbar>
...
export default {
  ...
  methods: {
    loadItems: function () {
      axios
        .get("api/fitoproducts")
        .then((response) => {
          this.fitoproducts = response.data;
        })
        .catch((e) => {});
    },
    ...
    updateItem: function () {
      axios
        .put("api/fitoproducts/" + this.showedItem.uuid, {
    ...
    editItem(item) {
      this.showedIndex = this.fitoproducts.indexOf(item);
    ...
    deleteItem(item) {
        axios
            .delete("api/fitoproducts/" + item.uuid)
            .then((response) => {
                this.close();
                this.$store.commit("snackbar/setSnack", response.data);
                this.loading = false;
