      ...
      <template v-slot:top class="toolbar">
       <div class="toolbar">
          ...
          <v-btn
            ...
            dark
            @click="showNewItem()">
            <v-icon dark left>mdi-plus</v-icon>Új parcella
          </v-btn>
       </div>
    ...
    <v-dialog v-model="show_dialog" width="750px">
      <v-card>
        <v-toolbar dense dark color="#292929">
          <v-toolbar-title>Parcella adatai</v-toolbar-title>
        </v-toolbar>
...
export default {
  ...
  data() {
    ...
    headers: [
        ...
        { text: "Csoport név", value: "group_name"},
        { text: "Rétegek száma", value: "layers_count"},
        { text: "Készítve", value: "created_at"},
        { text: "Frissítve", value: "updated_at"},
  ...
  methods: {
    ...
    updateItem: function () {
      axios
        .put("api/parcels/" + this.showedItem.uuid, {
          name: this.showedItem.name,
          data: JSON.stringify({"key" : 123}),
          identifier: "asdasdasdassda",
          group_uuid: this.showedItem.group_uuid
        })
        .then((response) => {
          this.close();
    ...
    addNewItem: function () {
      ...
      axios
        .post("api/parcels", {
          ...
          data: JSON.stringify({"key" : 123}),
          identifier: "asdasdasdassda",
          group_uuid: this.newItem.group_uuid
        })
        .then((response) => {
          this.close();
          this.$store.commit("snackbar/setSnack", response.data);
    ...
    editItem(item) {
      this.showedIndex = this.parcels.indexOf(item);
    ...
    toParcelDetails: function (event, item) {
        console.log(item)
        this.$router.push('/dashboard/parcels/' + item.item.uuid);
    },
  },
