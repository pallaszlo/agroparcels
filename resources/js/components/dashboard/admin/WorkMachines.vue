<template>
    <div>
        <section-header text="Munkaeszközök" />
        <v-data-table :headers="headers" :items="workMachines" :search="search" :no-data-text="nodata" :page.sync="page"
            ...
            <template v-slot:top class="toolbar">
                <div class="toolbar">
                    ...
                    <v-btn class="text-none new" tile dark @click="showNewItem()">
                        <v-icon dark left>mdi-plus</v-icon>Új Munkaeszköz
                    </v-btn>
                </div>
        ...
        <v-dialog v-model="show_dialog" width="750px">
            <v-card>
                <v-toolbar dense dark color="#292929">
                    <v-toolbar-title>Munkaeszköz adatai</v-toolbar-title>
                </v-toolbar>
                ...
                <v-card-actions>
                    ...
                    <v-btn :loading="loading" class="text-none new" tile dark @click.native="validateItem">Munkaeszköz
                        frissítése</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="show_dialog2" width="750px">
            <v-card>
                <v-toolbar dense dark color="#292929">
                    <v-toolbar-title>Új Munkaeszköz</v-toolbar-title>
                </v-toolbar>
                ...
                <v-card-actions>
                    <div class="flex-grow-1"></div>
                    <v-btn color="red darken-1" outlined @click.native="close">Bezár</v-btn>
                    <v-btn :loading="loading" class="text-none new" tile dark @click.native="validateNewItem">
                        Munkaeszköz létrehozása</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
...
export default {
    ...
    methods: {
        ...
        updateItem: function () {
            axios
                .put("api/workmachines/" + this.showedItem.uuid, {
        ...
        editItem(item) {
            this.showedIndex = this.workMachines.indexOf(item);
        ...
        deleteItem(item) {
            axios
                .delete("api/workmachines/" + item.uuid)
                .then((response) => {
                    this.close();
                    this.$store.commit("snackbar/setSnack", response.data);
                    this.loading = false;
