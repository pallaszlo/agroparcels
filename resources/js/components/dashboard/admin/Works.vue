<template>
    <div>
        <section-header text="Munkálatok" />
        <v-data-table :headers="headers" :items="works" :search="search" :no-data-text="nodata" :page.sync="page"
            ...
            <template v-slot:top class="toolbar">
                <div class="toolbar">
                    ...
                    <v-btn class="text-none new" tile dark @click="showNewItem()">
                        <v-icon dark left>mdi-plus</v-icon>Új Munkálat
                    </v-btn>
                </div>
        ...
        <v-dialog v-model="show_dialog" width="750px">
            <v-card>
                <v-toolbar dense dark color="#292929">
                    <v-toolbar-title>Munkálat adatai</v-toolbar-title>
                </v-toolbar>
                ...
                <v-card-actions>
                    ...
                    <v-btn :loading="loading" class="text-none new" tile dark @click.native="validateItem">Munkálat
                        frissítése</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="show_dialog2" width="750px">
            <v-card>
                <v-toolbar dense dark color="#292929">
                    <v-toolbar-title>Új Munkálat</v-toolbar-title>
                </v-toolbar>
                ...
                <v-card-actions>
                    <div class="flex-grow-1"></div>
                    <v-btn color="red darken-1" outlined @click.native="close">Bezár</v-btn>
                    <v-btn :loading="loading" class="text-none new" tile dark @click.native="validateNewItem">Munkálat
                        létrehozása</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
...
export default {
    ...
    data() {
        return {
            headers: [
                ...
                {
                    text: "Kezelés szükséges?",
                    align: "left",
                    value: "is_treatment_required",
                },
                {
                    text: "Történik változás?",
                    value: "culture_action"
                },
    ...
    methods: {
        ...
        updateItem: function () {
            axios
                .put("api/works/" + this.showedItem.uuid, {
