...
import TablePagination from '../admin/compnents/TablePagination.vue';
...
export default {
    ...
    data() {
        return {
            ...
            displaySettings: {
                works: {
                    edit: false,
                    buttonText: "Munkálatok hozzáadása",
                    route1: "persons/",
                    route2: "/works/"
                },
                work_machines: {
                    edit: false,
                    buttonText: "Új munkaeszköz",
                    route1: "works/",
                    route2: "/workmachines/"
                },
                operations: {
                    edit: true,
                    buttonText: "Új munka végzés",
                    route1: "persons/",
                    route2: "/operations/"
                }
            },
    ...
    methods: {
        loadItems: function () {
            axios.get('/api/persons/' + this.$store.state.auth.user.person_uuid)
                .then(response => {
        ...
        setUpNewWork: function () {
            axios.get('/api/persons/' + this.$store.state.auth.user.person_uuid + '/works/new')
                .then(response => {
